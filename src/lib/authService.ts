...
export async function getUserProfile(user: User | null): Promise<AuthProfile | null> {
  ...
  try {
    ...
    // Bootstrap is restricted to the configured platform owner.
    // Other pre-created Firebase Auth accounts must have an explicit RBAC document.
    ...
    if (!adminSnap.exists() && normalizedEmail === MASTER_SUPER_ADMIN_EMAIL) {
      try {
        ...
        // Ignore and continue; Firestore rules remain authoritative.
      }
    }
