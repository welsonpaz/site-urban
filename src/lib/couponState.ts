...
export async function restoreDefaultCouponsInDB(restaurantId?: string): Promise<Coupon[]> {
  try {
    const scopedCoupons = restaurantId
      ? DEFAULT_COUPONS.map((c) => ({ ...c, restaurantId }))
      : DEFAULT_COUPONS;
    ...
    localStorage.setItem('coupons_initialized', 'true');
    localStorage.setItem(COUPONS_STORAGE_KEY, JSON.stringify(scopedCoupons));
    window.dispatchEvent(new Event('coupons-updated'));
    return scopedCoupons;
  } catch (err) {
...
// A coupon applies to a restaurant when it has no restaurantId (global/legacy coupon)
// or when its restaurantId matches the one being viewed.
export function isCouponApplicable(coupon: Coupon, restaurantId?: string): boolean {
  if (!coupon.restaurantId) return true;
