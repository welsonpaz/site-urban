...
import { ScreenType, UserAddress, Restaurant } from '../types';
import { useLogo } from '../lib/logoState';
import { getCustomerByPhone, saveCustomerProfile, normalizeCustomerPhone } from '../lib/firebase';
...
export default function RegisterScreen({
  ...
  useEffect(() => {
    const cleanPhone = normalizeCustomerPhone(phoneInput);
    ...
    if (cleanPhone.length >= 10 && cleanPhone.length <= 11) {
      ...
      try {
        ...
        if (cached) {
          ...
          if (parsed && normalizeCustomerPhone(parsed.phone || '') === cleanPhone) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Salva o cliente no Firestore antes de liberar o checkout.
    // O documento é identificado pelo telefone normalizado, permitindo
    // recuperar os dados automaticamente nos próximos pedidos.
    const normalizedPhone = normalizeCustomerPhone(phoneInput);
    try {
      const saved = await saveCustomerProfile(normalizedPhone, {
      ...
      if (!saved) {
        setErrorMsg('Não foi possível salvar seus dados. Verifique sua conexão e tente novamente.');
        return;
      }

      // Cache local apenas para acelerar a próxima busca no mesmo aparelho.
      try {
        localStorage.setItem('urban_customer_profile', JSON.stringify({
          phone: normalizedPhone,
          name: nameInput.trim(),
          cep: cepInput.trim(),
          street: streetInput.trim(),
        ...
        // O Firebase continua sendo a fonte principal dos dados.
      }
    } catch (err) {
