...
import { ArrowLeft, Settings, Tag, Palette, Store, Lock, LogIn, Plus, Trash2, Package } from 'lucide-react';
...
export default function SuperAdminScreen({ onBack }: SuperAdminScreenProps) {
  ...
  // Sincroniza inputs de branding quando o estado global carregar
  useEffect(() => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6 text-gray-100">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-xl space-y-6">
        ...
        {/* ABA 1: GERAL (CADASTRO E GESTÃO DE PRODUTOS E FOTOS) */}
        {activeTab === 'config' && (
          <div className="space-y-6">
            <div>
              <h2 className="text-lg font-semibold text-white">Gerenciamento do Cardápio e Produtos</h2>
              <p className="text-sm text-gray-400">Cadastre novos itens, adicione URLs de fotos e remova produtos do cardápio.</p>
            </div>
            ...
            <form onSubmit={handleAddProduct} className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-950 p-4 rounded-xl border border-gray-800">
              ...
              <button type="submit" className="sm:col-span-2 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl text-sm transition-colors flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20">
                <Plus className="w-4 h-4" /> Cadastrar Produto com Foto
              </button>
            </form>
        ...
        {/* ABA 3: APARÊNCIA (NOME DO RESTAURANTE E LOGO) */}
        {activeTab === 'branding' && (
          <form onSubmit={handleSaveBranding} className="space-y-6">
            <div>
              <h2 className="text-lg font-semibold text-white">Identidade Visual, Nome e Logo</h2>
              <p className="text-sm text-gray-400">Mude o nome do restaurante e atualize o logotipo oficial do sistema.</p>
            </div>
            ...
            <div className="space-y-4 bg-gray-950 p-4 rounded-xl border border-gray-800">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">URL do Logotipo do Restaurante</label>
                <input 
                  type="url" 
                  value={logoInput} 
              ...
              {logoInput && (
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <img src={logoInput} alt="Logo Preview" className="w-10 h-10 object-contain bg-gray-900 rounded-lg border border-gray-800 p-1" onError={(e)=>{(e.target as HTMLElement).style.display='none'}} />
                </div>
              )}

              <button type="submit" className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl text-sm transition-colors flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20">
                <Palette className="w-4 h-4" /> Salvar Alterações de Aparência
              </button>
            </div>
          </form>
        )}
