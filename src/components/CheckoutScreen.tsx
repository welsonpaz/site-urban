...
import { saveCustomerProfile /* customer profile only */, normalizeCustomerPhone } from '../lib/firebase';
...
export default function CheckoutScreen({
  ...
  const handlePlaceOrder = () => {
    ...
    setTimeout(async () => {
      ...
      // Sempre atualiza o cadastro do cliente com os dados usados neste pedido.
      // Assim, no próximo pedido, basta informar o telefone para recuperar
      // nome e endereço salvos no Firestore.
      if (normalizedPhone.length >= 10 && customerName.trim() && address.street.trim()) {
        await saveCustomerProfile(normalizedPhone, {
          phone: normalizedPhone,
          name: customerName.trim(),
          street: address.street.trim(),
          details: address.details?.trim() || '',
          neighborhood: address.neighborhood.trim(),
          cityState: address.cityState?.trim() || '',
